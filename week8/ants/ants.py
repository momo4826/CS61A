...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            elif not place.ant.is_container and self.is_container and not self.ant_contained:
                self.store_ant(place.ant)
                self.place = place
                place.ant.place = self.place
                place.ant = self
            else:
    ...
    def nearest_bee(self):
        ...
        while True:
            ...
            if self.upper_bound and count > self.upper_bound:
                return None
            if self.lower_bound and count < self.lower_bound:
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.ant_contained:
            # print('place:', self.ant_contained.place is self.place)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        if gamestate.already_queen:
            return None
        else:
            gamestate.already_queen = True
            return super().construct(gamestate) # you have to return the value!!!!! Or it will return None
    ...
    def action(self, gamestate):
        ...
        while next_place:
            if next_place.ant and not next_place.ant.is_doubled:
                next_place.ant.double()
                # don't forget to double ant contained in container!!!!
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if not self.place.ant or self.place.ant.blocks_path == False or (self.place.ant.is_container and self.place.ant.ant_contained.blocks_path == False):
            return False
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        res = {}
        while next_place:
            if next_place.ant and not (next_place.ant is self):
                res[next_place.ant] = count
            if next_place.bees:
                for item in next_place.bees:
                    res[item] = count
            next_place = next_place.entrance
            count += 1
        return res
    ...
    def calculate_damage(self, distance):
        return self.damage - self.away_weaken * distance - self.insects_shot * self.damage_weaken
