...
def always_roll(n):
    ...
    assert n >= 0 and n <= 10
    # BEGIN PROBLEM 6
    def always_roll_helper(score, opponent_score):
        return n 
    return always_roll_helper
    # END PROBLEM 6
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    cmp = tail_points(opponent_score)
    if cmp >= threshold:
        return 0
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # try roll 0, if the score with this incresement will be higher than GOAL, then choose it
    cmp = tail_points(opponent_score)
    new_score = score + cmp
    if perfect_square(new_score):
        new_score = next_perfect_square(new_score)
    # print("here")
    # print(make_averaged(roll_dice, total_samples)(6, dice))
    if new_score >= GOAL or new_score - score >= 12:
        return 0
