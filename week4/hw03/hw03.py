...
def pingpong(n):
    ...
    if n == 1:
        ...
        if direction_recorder(n) > 0:
            return pingpong(n-1) + 1
        else:
            return pingpong(n-1) - 1

def direction_recorder(n):
    """1 means increasement, -1 means decreasement"""
    if n<= 8 :
        return 1
    else:
        if (n-1) % 8 == 0 or num_eights(n-1) > 0:
            return -direction_recorder(n-1)
        else:
            return direction_recorder(n-1)
...
def count_coins(change):
    ...
    True
    """
    def count_coins_less_than(n, m):
        if n < 0 or not m or m <= 0:
            return 0
        elif n == 0:
            return 1
        else:
            return count_coins_less_than(n-m, m) + count_coins_less_than(n, next_smaller_coin(m))
    return count_coins_less_than(change, 25)
